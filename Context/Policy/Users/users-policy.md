...
Если нужно создать пользователя, сменить пароль, отключить учетную запись, добавить пользователя в группу или изменить роль, это делается в `Authentik`.

В приложении для этого используется административная страница, которая ведет в `Authentik Admin UI`.
...
Текущий admin UI приложения не является полноценным user-management UI.

Он должен:

- объяснять, что пользователи управляются в `Authentik`
- давать ссылку на `Authentik Admin UI`
- не создавать ложное впечатление, что приложение само управляет пользователями

Пока не принято отдельное решение, в приложении не нужно добавлять:
...
Допустимо добавлять UI для app-specific пользовательских данных, если они хранятся в `UserProperties` и не затрагивают identity.
