...
#### Users, roles, and content access
- Добавлена модель пользователей, ролей, групп, назначений ролей и эффективных прав с хранением в PostgreSQL и административным UI.
- Введена политика доступа к документам и rich-документам: дерево, страницы документов и выбор пространств фильтруются по правам текущего пользователя.
- Добавлен анонимный пользователь, анонимный вход, выбор доступных пространств и профиль пользователя.

#### Document printing
- Добавлена отдельная страница печати для обычных документов и rich-документов с toolbar-командами печати на принтер и печати в PDF.
- Параметры печати вынесены в пользовательские настройки: масштаб шрифта, поля страницы и именованные пресеты с сохранением, выбором и удалением.

#### Rich-document import and editing
- Clipboard-вставка rich-документа переведена на отдельный import helper с детектором содержимого буфера.
- Markdown import и paste теперь преобразуют таблицы, fenced code blocks, inline code и markdown-списки в rich-document blocks редактора.

#### Deployment
- Добавлен минимальный self-hosted deployment bundle для Windows/Docker Compose: install scripts, compose-файлы, env-шаблон и bootstrap начальных данных.
- Добавлен PowerShell-сценарий сборки release bundle из текущего репозитория.

### MINOR-FEATURES

#### Rich-document UI
- Обновлен блок содержания rich-документа: номера страниц стали компактными, а обновление содержания вынесено в icon-only кнопку.
- Исправлены сценарии создания rich-документа из дерева: новый документ выделяется и сразу открывается на редактирование.

#### Navigation and tree
- В дереве имена элементов зафиксированы в одну строку с обрезкой и tooltip полного имени при наведении.
- В пользовательское меню добавлена подменюшка пространств, быстрый переход между пространствами и пункт `Все пространства`.

#### User administration
- В назначениях ролей добавлен режим показа всех назначений независимо от выбранного пространства.

#### Public project documentation
- Добавлены публичные README на русском и английском, описание текущих функций, trademark-документы и политика публикации проекта на GitHub.
- Уточнена модель участия: основной репозиторий принимает feedback через Issues, без ожидания pull requests.

#### Maintenance
- Удалены оставшиеся аллюзии на старую систему `SampleOnlineMall`; namespace веб-логгера переведен в домен `BusinessEntity`.
- Убраны временные диагностические файлы и старые служебные артефакты из репозитория.
...
#### MediaServer MiniApp
- Медиатека переведена на async upload jobs для видео: несколько файлов загружаются параллельно, прогресс хранится на сервере и продолжает отображаться после переоткрытия вкладки.
- Видео стали space-scoped бизнес-сущностями: при загрузке создается связь `RelatesTo` между текущим Space и `MediaVideo`, а медиатека и rich-doc Embed работают только с видео текущего пространства.
- Для `MediaVideo` добавлен пользовательский комментарий, который сохраняется в `BusinessEntityData` и metadata видео.

#### Multimedia UI
- В медиатеке добавлен раскрываемый подвал записи: клик по видео открывает inline-плеер и поле комментария, повторный клик закрывает просмотр.
- Добавлен фильтр видео по подстроке в отображаемом имени, имени файла или комментарии; список перечитывается без ограничения длины результата.

### MINOR-FEATURES

#### Media upload UX
- Завершенные upload progress bars больше не остаются в списке после успешной загрузки.
- Кнопки сохранения имени и комментария заменены с галочки на иконку дискеты.
- Кнопка фильтра оформлена как icon-only control с funnel-иконкой.

#### Rich-text document embed
- В rich-doc Embed список видео и загрузка нового видео используют текущий Space, чтобы вставка не смешивала мультимедиа разных пространств.

#### Maintenance
- Уточнен сценарий `RSTT`: если код не менялся с прошлого перезапуска, контейнеры не пересобираются.
