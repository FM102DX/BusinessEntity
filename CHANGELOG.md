...
## [0.16.1] - 2026-06-14 18:22:49 +03:00

### MAJOR-FEATURES

#### Document and rich-document actions
- В меню действий обычного документа и rich-text документа добавлена кнопка `Удалить`, которая удаляет документ через общий tree-service и обновляет дерево без ручного перезахода.
- Страницы обычного документа и rich-text документа теперь синхронизируют выбранный узел дерева с открытым документом: родительские папки раскрываются, а соответствующий пункт подсвечивается как выбранный.

### MINOR-FEATURES

#### Document and rich-document layout
- Новый верхний тулбар документов упразднен: действия документов снова собраны компактным блоком в правой колонке рядом с панелью поиска/версий, чтобы не перекрывать область чтения.
- После проверки эксперимента с отдельными иконками пустых папок дерево возвращено к штатным иконкам `folder` / `folder_open`.

#### Deployment policy
- Политика деплоя и связанные материалы вынесены в отдельную папку `Context/Policy/Deployment`, а индекс контекста обновлен на новый путь.

## [0.16.0] - 2026-06-13 18:27:23 +03:00

### MINOR-FEATURES

#### Document and rich-document toolbars
- Действия обычного документа в режимах чтения и редактирования перенесены в верхний тулбар, чтобы кнопки не висели сбоку от содержимого.
- Действия rich-text документа в read/edit режимах вынесены в отдельный верхний тулбар над заголовком, а правый блок действий в header удален.
- Тулбары получили адаптивное поведение: на узких экранах действия переносятся строками вместо выхода за границы области документа.

## [0.15.2] - 2026-06-10 19:18:34 +03:00

### MAJOR-FEATURES

#### Rich-text document editing
- Создание rich-text документа из контекстного меню дерева теперь открывает новый документ сразу в режиме редактирования: query-параметр `edit=1` корректно применяется страницей документа.
- Отложенное открытие документа по клику в дереве отменяется перед context-menu action, чтобы оно не перебивало edit-навигацию нового rich-text документа.
- При переходе из редактирования rich-text документа в режим чтения read-view перезагружает актуальное окно чанков последней версии вокруг текущей позиции редактора.

### MINOR-FEATURES

#### Rich-text document outline
- Секция содержания rich-text документа сжата примерно на 20% в пользу основного текста: левая колонка уменьшена до `176-224px`, уменьшены отступы и управляющие кнопки.
- Текст содержания в read/edit режимах выровнен влево, переведен в верхний регистр и уменьшен до компактного рабочего размера.

#### Document edit links
- Query-параметр `edit` явно подключен как параметр Blazor-страницы для обычных и rich-text документов.
...
#### Rich-document comments tab
- Комментарии rich-документа перенесены из нижней области страницы в отдельную вкладку `Комм.` правого tab-control.
- Редактор комментария уплотнен: действие отправки отображается маленькой ссылкой под полем ввода вместо кнопки рядом с полем.

## [0.14.8] - 2026-06-02 16:48:08 +03:00

### MINOR-FEATURES

#### Comments UI
- У секции комментариев убран текстовый заголовок, чтобы не занимать место под rich-документом.
- Редактор комментария сделан компактнее: поле ввода уменьшено до двух строк, а кнопка отправки размещена справа от поля.
...
#### Rich-document comments layout
- В read-mode rich-документа секция комментариев получила зарезервированную нижнюю область без отдельного scroll-контейнера списка комментариев.
- Механика прокрутки текста rich-документа оставлена во внутреннем viewport rich-doc и отделена от прокрутки окна страницы.

#### Application shell scroll
- Общий scrollbar окна теперь прокручивает shell страницы целиком, включая левую навигацию, дерево, центральную область и правую панель.
- Sticky/fixed-height поведение боковых панелей и rich-doc колонок заменено на обычную высоту страницы, чтобы оно не конкурировало с прокруткой rich-документа.
