...
$imageName = "web_logger_image"
$containerName = "web_logger-container"
...
docker run -e 'ASPNETCORE_URLS=http://*:80' --network docker-networkmall2   -e 'ASPNETCORE_ENVIRONMENT=Development' -d --name $containerName -p "$($portExt):$($portInt)" $imageName
