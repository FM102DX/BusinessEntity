...
public class InMemoryRepository<T> : BusinessEntity.MiniApps.DataProviderMiniApp.Contracts.Repositories.IAsyncRepository<T> where T : class, IBaseEntity
{
    ...
    public Task<IReadOnlyList<T>> GetAllAsync(Expression<Func<T, bool>>? filter = null, int? take = null, CancellationToken ct = default)
    {
        lock (_lock)
        {
            ...
            if (take.HasValue) query = query.OrderByDescending(x => x.CreatedDate).Take(take.Value);
    ...
    public Task<bool> ExistsAsync(Guid id, CancellationToken ct = default) => Task.FromResult(_storage.ContainsKey(id));
    ...
    public Task<T> AddAsync(T entity, CancellationToken ct = default)
    {
        ...
        lock (_lock)
        {
            ...
            if (!_storage.TryAdd(entity.Id, entity)) throw new InvalidOperationException($"Entity with ID {entity.Id} already exists");
            return Task.FromResult(entity);
        }
    }
    ...
    public Task UpdateAsync(T entity, CancellationToken ct = default)
    {
        ...
        lock (_lock)
        {
            if (!_storage.ContainsKey(entity.Id)) throw new KeyNotFoundException($"Entity with ID {entity.Id} not found");
            entity.LastModifiedDate = DateTime.UtcNow;
    ...
    public Task DeleteAsync(Guid id, CancellationToken ct = default)
    {
        lock (_lock)
        {
            if (!_storage.TryRemove(id, out _)) throw new KeyNotFoundException($"Entity with ID {id} not found");
            return Task.CompletedTask;
        }
    }

    public Task<int> GetCountAsync(CancellationToken ct = default) => Task.FromResult(_storage.Count);
